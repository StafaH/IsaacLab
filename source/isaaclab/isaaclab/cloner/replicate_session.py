...
def replicate(plan: ClonePlan, *, stage: Usd.Stage, replicate_physics: bool = True) -> None:
    ...
    visualizers, and non-Newton renderers retain USD replication. Explicit contexts are
    honored regardless of Kit availability. With ``replicate_physics=False`` physics contexts
    are dropped and USD replication is retained when requested explicitly or Kit is available.
